...
- 工具栏为 ZStack 叠层：底层是上半环角向渐变淡蓝色块（以底边中点为圆心只扫上半圈，左右对称、中心拱顶最深、向两侧渐浅，`ToolbarRainbowBackground`）；上层磨砂工具栏完全覆盖色块（同为 378×48pt 全圆角）。系统玻璃材质（`NSVisualEffectView` / `glassEffect`）只采样窗口后方内容，无法模糊同一窗口内的彩虹背景，因此玻璃层内绘制与底层像素对齐的彩虹副本并施加 `.blur(radius: 12)`，再叠 75% 磨砂白，模拟高斯模糊采样效果。左侧拖动点阵为黑色，复制、翻译、解释均显示黑色文字；“深度理解”“润色”以黑色禁用态展示，等待后续实现。
- 分区线使用带水平内边距的 0.5pt 弱分隔线，不与玻璃或内容层边缘相接。
- 长文本内容区使用白色磨砂微透明背景；继续提问输入框保持更高不透明度，避免输入控件丢失边界和对比度。

项目是原生 SwiftUI / AppKit 应用，Flutter 的 `liquid_glass_widgets` 无法直接作为 Swift Package 接入；结果侧栏和菜单栏继续使用系统玻璃兼容层，工具栏使用上半环淡蓝色角向渐变背景 + 模糊副本磨砂玻璃，前景文字由 SwiftUI 绘制。
